...
if __name__ == "__main__":
    ...
    KNOWN_CHARS = set().union(
        *(set(m.original_title) for mss in ms.values() for m in mss)
    )
    KNOWN_CHARS_KC = set().union(
        *(
            set(unicodedata.normalize("NFKC", m.original_title))
            for mss in ms.values()
            for m in mss
        )
    )
    KNOWN_CHARS_KD = set().union(
        *(
            set(unicodedata.normalize("NFKD", m.original_title))
            for mss in ms.values()
            for m in mss
        )
    )
